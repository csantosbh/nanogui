...
void Texture3D::init() {
    ...
    for (int i = 0; i<2; ++i) {
        switch (i == 0 ? m_min_interpolation_mode : m_mag_interpolation_mode) {
            ...
            default: throw std::runtime_error("Texture3D::Texture3D(): invalid interpolation mode!");
        }
    }
    ...
    switch (m_wrap_mode) {
        ...
        default: throw std::runtime_error("Texture3D::Texture3D(): invalid wrap mode!");
    }
...
void Texture3D::upload(const uint8_t *data) {
    GLenum pixel_format_gl,
           component_format_gl,
           internal_format_gl;
...
#endif
        CHK(glTexImage3D(tex_mode, 0, internal_format_gl, (GLsizei) m_size.x(),
             (GLsizei) m_size.y(), (GLsizei) m_size.z(), 0, pixel_format_gl,
             component_format_gl, data));
    } else {
        throw std::runtime_error("Texture3D::upload() called without data buffer!");
    }
}

void Texture3D::download(uint8_t *data) {
#if defined(NANOGUI_USE_GLES)
    (void) data;
    throw std::runtime_error("Texture3D::download(): not supported on GLES 2!");
#else
    if (m_texture_handle == 0)
        throw std::runtime_error("Texture3D::download(): no texture handle!");
    ...
    CHK(glBindTexture(GL_TEXTURE_3D, m_texture_handle));
    CHK(glGetTexImage(GL_TEXTURE_3D, 0, pixel_format_gl, component_format_gl, data));
#endif
}

void Texture3D::resize(const Vector3i &size) {
    if (m_size == size)
        return;
    m_size = size;
    upload(nullptr);
