...
static py::array cubemap_download(CubeMap &texture) {
    const char *dtype_name;
    switch (texture.component_format()) {
        case Texture::ComponentFormat::UInt8:   dtype_name = "u1"; break;
        case Texture::ComponentFormat::Int8:    dtype_name = "i1"; break;
...
static void cubemap_upload(CubeMap &texture, py::array array) {
    size_t n_channels = array.ndim() == 4 ? array.shape(3) : 1;
    VariableType dtype         = dtype_to_enoki(array.dtype()),
                 dtype_texture = (VariableType) texture.component_format();

    if (array.ndim() != 3 && array.ndim() != 4)
        throw std::runtime_error("CubeMap::upload(): expected a 3 or 4-dimensional array!");
    else if(array.shape(0) != 6)
        throw std::runtime_error("CubeMap::upload(): expected input to be of shape (6, height, width)!");
    else if (array.shape(1) != texture.size().y() ||
             array.shape(2) != texture.size().x())
        throw std::runtime_error("CubeMap::upload(): array size does not match the texture!");
    else if (n_channels != texture.channels())
        throw std::runtime_error(
            std::string("CubeMap::upload(): number of color channels in array (") +
            std::to_string(n_channels) + ") does not match the texture (" +
            std::to_string(texture.channels()) + ")!");
    else if (dtype != dtype_texture)
        throw std::runtime_error(
            std::string("CubeMap::upload(): dtype of array (") +
            type_name(dtype) + ") does not match the texture (" +
            type_name(dtype_texture) + ")!");
...
void register_render(py::module &m) {
    ...
    auto texture3d = py::class_<Texture3D, Object, ref<Texture3D>>(m, "Texture3D", D(Texture3D));
    ...
    auto cubemap = py::class_<CubeMap, Object, ref<CubeMap>>(m, "CubeMap", D(CubeMap));

    cubemap
        .def(py::init<PixelFormat, ComponentFormat, const Vector2i &,
                      InterpolationMode, InterpolationMode, WrapMode>(),
             D(CubeMap, CubeMap), "pixel_format"_a, "component_format"_a, "size"_a,
             "min_interpolation_mode"_a = InterpolationMode::Bilinear,
             "mag_interpolation_mode"_a = InterpolationMode::Bilinear,
             "wrap_mode"_a = WrapMode::ClampToEdge)
        .def("pixel_format", &CubeMap::pixel_format, D(CubeMap, pixel_format))
        .def("component_format", &CubeMap::component_format, D(CubeMap, component_format))
        .def("min_interpolation_mode", &CubeMap::min_interpolation_mode, D(CubeMap, min_interpolation_mode))
        .def("mag_interpolation_mode", &CubeMap::mag_interpolation_mode, D(CubeMap, mag_interpolation_mode))
        .def("wrap_mode", &CubeMap::wrap_mode, D(CubeMap, wrap_mode))
        .def("size", &CubeMap::size, D(CubeMap, size))
        .def("bytes_per_pixel", &CubeMap::bytes_per_pixel, D(CubeMap, bytes_per_pixel))
        .def("channels", &CubeMap::channels, D(CubeMap, channels))
        .def("download", &cubemap_download, D(CubeMap, download))
        .def("upload", &cubemap_upload, D(CubeMap, upload))
        .def("resize", &CubeMap::resize, D(CubeMap, resize))
        .def("texture_handle", &CubeMap::texture_handle)
    ;
    ...
    shader
        ...
        .def("set_cubemap", &Shader::set_cubemap, D(Shader, set_cubemap))
